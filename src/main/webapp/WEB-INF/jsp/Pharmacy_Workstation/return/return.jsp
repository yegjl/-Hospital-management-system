                        ...
                        <div class="layui-btn-group" style="margin-left: 70%;">
                            <div class="layui-fluid" id="LAY-component-layer-special-demo">
                                <div class="layui-btn-container layadmin-layer-demo">
                                    <button data-method="tuiyao" class="layui-btn layui-btn-sm layui-btn-normal">退药</button>
                                    <button data-method="quantui" class="layui-btn layui-btn-sm layui-btn-danger">全退</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    layui.config({
        ...
        var active = {
            //TODO:退的时候drugs的相应数量也改变，已经发了的就是加上退掉的,然后开药部分也要做对应drugs的处理
            tuiyao: function () {
                var that = this;
                var tabledata = layui.table.checkStatus('test-table-simple').data;
                var tabledata2 = layui.table.checkStatus('test-table-simple2').data;
                if (tabledata2[0].dispensestatus == 0) {
                    alert("还未发药，不能退！");
                    return;
                }
                // var iid = tabledata[0].id;
                layer.open({
                    type: 1,
                    title: '退药',
                    area: ['300px', '200px'],
                    ...
                    content: '<iframe src="drugstore/indexmakereturn?id='+tabledata[0].id+'" frameborder="0" class="layadmin-iframe"></iframe>',
                    btn: ['确定', '取消'],
                        yes: function (index,layero) {
                            var iframes = $(layero).find("iframe")[0].contentWindow;
                            // var form = iframes.document.getElementById("tuiyao");
                            var amount = iframes.document.getElementById("amount").value;//现在数量
                            var reamount = iframes.document.getElementById("renum").value;//退掉数量
                            if (reamount > amount) {
                                alert("退药量多于总量，无法退药");
                                return;
                            }
                            $.ajax({
                                type: "POST",
                                url: "drugstore/returnmedicine?id="+tabledata[0].id+"&amount="+amount+"&renum="+reamount,
                            ...
                            $.ajax({
                                type: "POST",
                                // Integer medicineid,Integer amount
                                url: "drugstore/returnamount?medicineid="+tabledata[0].medicineid+"&amount="+reamount,
                                success: function (res) {
                                    if (res.status == 0) {
                                    } else {
                                        layer.msg(res.message)
                            ...
                            $.ajax({
                                type: "POST",
                                url: "drugstore/sendtorefund?medicalrecordid="+tabledata[0].medicalrecordno+"&medicineid="+tabledata[0].medicineid+"&renum="+reamount,
                                success: function (res) {
                                    if (res.status == 0) {
                                    } else {
                                        layer.msg(res.message)
            ...
            quantui: function () {
                ...
                layer.msg('是否全退？', {
                    ...
                    yes: function () {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "drugstore/changestatus?medicalrecordno=" + recordid + "&status=2",//将状态改为已退的2
                            success: function (res) {
                                if (res.status == 0) {
                                } else {
                                    layer.msg(res.message)
                        ...
                        $.ajax({
                            ...
                            url: "drugstore/sendalltorefund?medicalrecordids=" + medicalrecordids + "&medicineids="+medicineids+"&renums="+amounts,
                            success: function (res) {
                                if (res.status == 0) {
                                } else {
                                    layer.msg(res.message)
